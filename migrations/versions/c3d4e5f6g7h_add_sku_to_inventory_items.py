...
def upgrade():
    conn = op.get_bind()
    cols = [row[1] for row in conn.execute(sa.text('PRAGMA table_info("inventory_items")'))]
    if 'sku' not in cols:
        op.add_column('inventory_items', sa.Column('sku', sa.String(length=64), nullable=False))
