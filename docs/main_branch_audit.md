...
## Executive verdict
The main branch is **closer but still not ready for production**. The import-blocking `bank_connections` collision is resolved **and the stock engine now runs cleanly under pytest** (see command output below). We stabilized the test harness and aligned ledger reference handling so UUID-backed inserts no longer crash under SQLite. A new preflight guard (`tools/check_migration_health.py`) now blocks dual migration roots or multi-head Alembic states, and repository logs are ignored by default to avoid accidental commits. The remaining freeze should stay in place until the broader suite (orders/finance/bots) is exercised in CI and post-deploy smoke checks are added.
...
3. **Migration safety gates** – The repository previously presented dual migration trees (`alembic/` and `migrations/`). A new guard script halts deploys when multiple roots or Alembic heads are detected; CI must wire it before promotion. 【F:tools/check_migration_health.py†L1-L83】
