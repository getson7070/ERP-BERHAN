...
def test_orders_flow(tmp_path, monkeypatch):
    ...
    with app.app_context():
        ...
        conn.execute(
            text(
                "CREATE TABLE orders (id INTEGER PRIMARY KEY AUTOINCREMENT, item_id INTEGER, quantity INTEGER, customer TEXT, sales_rep TEXT, vat_exempt BOOLEAN, status TEXT)"
            )
        )
    ...
    with app.app_context():
        ...
        conn.execute(
            text(
                "INSERT INTO orders (item_id, quantity, customer, sales_rep, vat_exempt, status) VALUES (1,1,'Bob','alice',0,'pending')"
            )
        )
        conn.commit()
