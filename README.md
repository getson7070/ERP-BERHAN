...
For AWS App Runner source-based deployments, an `apprunner.yaml` file specifies build and start commands. The configuration installs dependencies with `pip install -r requirements.txt`, runs migrations via `flask db upgrade`, and launches the service using `gunicorn --bind 0.0.0.0:${PORT:-8080} wsgi:app`. Ensure the service defines a `DATABASE_URL` (append `?sslmode=require`), `FLASK_SECRET_KEY`, `JWT_SECRETS`, and `REDIS_URL` environment variables.
