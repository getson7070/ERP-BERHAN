...
def index():
    conn = get_db()
    cur = conn.cursor()
    cur.execute('SELECT id, name FROM procurement_suppliers WHERE org_id = %s ORDER BY id', (session.get('org_id'),))
...
def add_supplier():
    if request.method == 'POST':
        ...
        cur.execute('INSERT INTO procurement_suppliers (org_id, name) VALUES (%s,%s)', (session.get('org_id'), name))
