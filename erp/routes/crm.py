...
def index():
    ...
    if customers is None:
        ...
        cur.execute('SELECT id, name FROM crm_customers WHERE org_id = %s ORDER BY id', (org_id,))
...
def add_customer():
    if request.method == 'POST':
        ...
        conn = get_db(); cur = conn.cursor()
        org_id = session.get('org_id')
        cur.execute('INSERT INTO crm_customers (org_id, name) VALUES (%s,%s)', (org_id, name))
