...
def _build_query(sku, sort, direction):
    query = Inventory.tenant_query()
    if sku:
...
def inventory_table():
    ...
    query = _build_query(sku, sort, direction)
...
def update_item(item_id):
    item = Inventory.tenant_query().filter_by(id=item_id).first_or_404()
