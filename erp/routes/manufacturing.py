...
def index():
    conn = get_db()
    cur = conn.cursor()
    cur.execute('SELECT id, name FROM manufacturing_jobs WHERE org_id = %s ORDER BY id', (session.get('org_id'),))
    jobs = cur.fetchall()
...
def add_job():
    if request.method == 'POST':
        ...
        cur.execute('INSERT INTO manufacturing_jobs (org_id, name) VALUES (%s,%s)', (session.get('org_id'), name))
